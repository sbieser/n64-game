...
Environmental rails. Eruption corridors channel the player between active volcanic flows. The path exists because the lava defines it: cool margins between flows, sulfur plains momentarily traversable before the next eruption reshapes them. The player is threading a landscape that is actively changing.

---

## Threat — Dangerous Heat

On the ice moon, cold was the threat and warmth was the resource. Here the inversion is exact.
...
Player death ghosts may be buried under fresh lava on subsequent visits — the eruption cycle is irregular. Some ghosts persist, others are gone under new material. Ghosts encased in cooled lava are static and dark, the lava having hardened around them. Still there, just entombed rather than tumbling.

A ghost preserved in sulfur looks different from one in lava: brighter color, the shape more intact. The planet's chemistry decides what each death looks like.

**Resource:** cool exhaust. The ghost's cooling systems were still running when it died — residual coolant can be drawn to reduce hull temperature. Same charge system as oxygen. See `mechanics/ghosts.md`.
